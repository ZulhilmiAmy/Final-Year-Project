...
function fetchAppointmentsAndInit() {
    fetch(window.appointmentsUpcomingUrl)
        ...
        .then(data => {
            ...
            buildPegawaiColors(allAppointments);   // ✅ assign warna ikut pegawai
            generatePegawaiButtons(allAppointments); // ✅ butang pegawai kes
            loadEventsToCalendar(allAppointments);  // ✅ load event default
            updateReminderList(allAppointments);    // ✅ update reminder list
        })
        .catch(err => console.error(err));
}

/* ===========================
   Assign warna untuk pegawai
   =========================== */
...
function generatePegawaiButtons(data) {
    ...
    pegawaiList.forEach(pg => {
        ...
        btn.innerText = pg;
        btn.style.backgroundColor = pegawaiColors[pg];
        btn.style.color = '#fff';
